...
class CnnGenerator(nn.Module):
    def __init__(self, nz=10, ngf=64, nc=1):
        ...
        self.convt_3 = nn.ConvTranspose2d(
            in_channels=2*ngf, out_channels=ngf, kernel_size=4, stride=10, bias=False, padding=0)
        self.convt_4 = nn.ConvTranspose2d(
            in_channels=ngf, out_channels=nc, kernel_size=3, stride=11, bias=False, padding=7)
...
class CnnDiscriminator(nn.Module):
    def __init__(self, nc=1, ndf=64):
        ...
        self.conv_1 = nn.Conv2d(
            in_channels=nc, out_channels=ndf, kernel_size=4, stride=2, bias=False, padding=1)
        self.conv_2 = nn.Conv2d(in_channels=ndf, out_channels=2 *
                                ndf, kernel_size=4, stride=2, bias=False, padding=1)
        self.conv_3 = nn.Conv2d(in_channels=2*ndf, out_channels=4 *
                                ndf, kernel_size=4, stride=2, bias=False, padding=1)
        self.conv_4 = nn.Conv2d(
            in_channels=4*ndf, out_channels=1, kernel_size=4, stride=2, bias=False, padding=1)
